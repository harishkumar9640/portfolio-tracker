...
PREV_FILE = DATA_DIR / "shareholding_prev.json"
LOG_FILE = DATA_DIR / "shareholding_alert_log.json"
...
def fetch_one(ticker: str, *, timeout: int = 30) -> Optional[TickerShareholding]:
    ...
    info = mf_holdings.TICKER_MAP.get(ticker)
    if not info:
        return None
    if info.get("id", 0) >= mf_holdings.PLACEHOLDER_ID_THRESHOLD:
