...
def tmp_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("mf_sgb.HistoryDB", lambda *a, **kw: HistoryDB(tmp_path / "test.db"))
...
class TestBuildIsinToNseSymbolMap:
    ...
    def test_extracts_both_isins(self, monkeypatch):
        ...
        monkeypatch.setattr(mf_sgb.requests, "get", lambda *a, **kw: mock_resp)
        result = mf_sgb._build_isin_to_nse_symbol_map()
    ...
    def test_handles_fetch_failure(self, monkeypatch):
        def boom(*a, **kw):
            raise mf_sgb.requests.RequestException("timeout")
        monkeypatch.setattr(mf_sgb.requests, "get", boom)
        result = mf_sgb._build_isin_to_nse_symbol_map()
        assert result == {}
    ...
    def test_is_cached_after_first_call(self, monkeypatch):
        ...
        monkeypatch.setattr(mf_sgb.requests, "get", counting_get)
        mf_sgb._build_isin_to_nse_symbol_map()
        mf_sgb._build_isin_to_nse_symbol_map()
...
class TestFetchNseQuote:
    ...
    def test_returns_price_for_known_isin(self, monkeypatch):
        monkeypatch.setattr(mf_sgb, "_fetch_nse_sgb_universe", lambda: NSE_UNIVERSE)
        monkeypatch.setattr(mf_sgb, "_build_isin_to_nse_symbol_map",
                            lambda: {"IN0020230184": "SGBFEB32IV"})
        result = mf_sgb._fetch_nse_quote("IN0020230184")
    ...
    def test_returns_none_for_unknown_isin(self, monkeypatch):
        monkeypatch.setattr(mf_sgb, "_fetch_nse_sgb_universe", lambda: NSE_UNIVERSE)
        monkeypatch.setattr(mf_sgb, "_build_isin_to_nse_symbol_map", lambda: {})
        result = mf_sgb._fetch_nse_quote("IN0000000000")
        assert result is None

    def test_returns_none_when_nse_unavailable(self, monkeypatch):
        monkeypatch.setattr(mf_sgb, "_fetch_nse_sgb_universe", lambda: None)
        result = mf_sgb._fetch_nse_quote("IN0020230184")
        assert result is None
    ...
    def test_universe_is_cached_for_5_minutes(self, monkeypatch):
        ...
        mf_sgb._NSE_SGB_DATA = NSE_UNIVERSE
        mf_sgb._NSE_SGB_FETCHED_AT = mf_sgb.time.monotonic()
        result = mf_sgb._fetch_nse_sgb_universe()
...
class TestSgbPricePriority:
    ...
    def test_nse_takes_priority_over_mintbyte(
        ...
        monkeypatch.setattr(mf_sgb, "fetch_mintbyte_with_history", lambda: {
            "IN0020230184": {"price": 15302.74, "date": "2026-06-22"},
        })
        monkeypatch.setattr(mf_sgb, "_fetch_ibja_gold_price",
                            lambda **kw: (14478.80, "23/06/2026"))
        monkeypatch.setattr(mf_sgb, "fetch_mf_rows", lambda mfs: [])
    ...
    def test_nse_price_persisted_to_history(
        ...
        before = mf_sgb.HistoryDB().sgb_history("IN0020230184")
        mf_sgb.fetch_sgb_rows([
            {"isin": "IN0020230184", "units": 1, "name": "SGB IV"},
        ])
        after = mf_sgb.HistoryDB().sgb_history("IN0020230184")
