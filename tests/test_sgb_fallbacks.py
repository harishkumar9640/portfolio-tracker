"""
Tests for mf_sgb.fetch_sgb_rows covering all 4 price-source fallbacks:
...
def reset_module_state():
    """Reset mf_sgb's module-level cache before every test."""
    mf_sgb._ISIN_TO_NSE_SYMBOL = {}
    mf_sgb._NSE_SGB_DATA = []
    mf_sgb._NSE_SGB_FETCHED_AT = 0.0
    mf_sgb._sgb_history_cache = None
...
def no_network_calls(monkeypatch):
    ...
    *default* behaviour is no-op.
    ...
    monkeypatch.setattr(mf_sgb, "_fetch_nse_sgb_universe", lambda: [])
...
class TestNSEPath:
    ...
    def test_nse_price_persisted_to_history(self):
        """Successful NSE lookups write to history_db for next time."""
        nse_universe = [{
            "symbol": "SGBFEB32IV", "ltP": "14950.00",
            "prevClose": "15129.77", "maturityDate": "B32IV",
        }]
        ...
        with patch.object(mf_sgb, "_fetch_nse_sgb_universe",
             ...
             patch.object(mf_sgb, "_build_isin_to_nse_symbol_map",
                          return_value=isin_map), \
             mp.object(mf_sgb, "HistoryDB",
                       lambda *a, **kw: HistoryDB(tmp_db_path)):
            mf_sgb.fetch_sgb_rows([_make_holding("IN0020230184")])
...
class TestMintbyteFallback:
    def test_mintbyte_used_when_nse_unavailable(self):
        ...
        mf_sgb._NSE_SGB_DATA = []   # empty NSE universe
        mf_sgb._NSE_SGB_FETCHED_AT = float("inf")
        mf_sgb._ISIN_TO_NSE_SYMBOL = {}   # no mapping

        with patch.object(mf_sgb, "fetch_mintbyte_with_history",
                          return_value={"IN0020230184":
                                       {"price": 15302.74, "date": "2026-06-22"}}), \
             patch.object(mf_sgb, "get_sgb_prev_price",
                          return_value=(15400.00, "2026-06-22")):
            rows = mf_sgb.fetch_sgb_rows([_make_holding("IN0020230184")])
...
class TestManualPriceFallback:
    def test_manual_used_when_nse_and_mintbyte_unavailable(self):
        ...
        with patch.object(mf_sgb, "fetch_mintbyte_with_history",
                          return_value={}):
            rows = mf_sgb.fetch_sgb_rows([_make_holding(
        ...
        with patch.object(mf_sgb, "fetch_mintbyte_with_history",
             ...
             patch.object(mf_sgb, "_fetch_ibja_gold_price",
                          side_effect=[(14478.80, "23/06/2026"),
                                       (14666.40, "22/06/2026")]):
            rows = mf_sgb.fetch_sgb_rows([_make_holding("IN0020999999")])
...
class TestMultipleHoldings:
    ...
    def test_skips_entries_with_zero_units(self):
        ...
        mf_sgb._NSE_SGB_FETCHED_AT = float("inf")
        mf_sgb._ISIN_TO_NSE_SYMBOL = {}
        with patch.object(mf_sgb, "fetch_mintbyte_with_history", return_value={}):
            rows = mf_sgb.fetch_sgb_rows([
